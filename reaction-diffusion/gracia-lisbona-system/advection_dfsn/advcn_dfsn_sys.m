...
%%%%%%%%%%%
 for i=1:Nx+1
    U1(i,1)=c1*exp(-x(i)/e1)+c2*exp(-x(i)/e2);
    U2(i,1)=c2*exp(-x(i)/e2)+x(i)*exp(x(i)-1);
end
 for j=1:Nt
    U1(1,j)=0;
    U1(Nx+1,j)=0;
...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=non1(e1,e2,x(i),t(j));
        %f2(i,j)=1+(x(i)*t(j)^2);
        f2(i,j)=non2(e1,e2,x(i),t(j));
end
end
%%%%%%%%%%%%%%%%%%%%
for j=1:Nt+1
    for i=1:Nx+1
       U1_exct(i,j)=exp(-t(j))*(c1*exp(-x(i)/e1)+c2*exp(-x(i)/e2));
       U2_exct(i,j)=exp(-t(j))*(c2*exp(-x(i)/e2)+x(i)*exp(x(i)-1));
    end
end
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*((1+x(i)^2)/h(i+1))+k*(2+(x(i))^2)+1;
     D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*(1+x(i)^2)/h(i+1);
 end
 i=1;
      D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*((1+x(i)^2)/h(i+1))+k*(2+(x(i))^2)+1;
     D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*(1+x(i)^2)/h(i+1);
