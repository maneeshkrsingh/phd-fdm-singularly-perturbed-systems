...
%%%%%%%%%%%
 for i=1:Nx+1
    U1(i,1)=c*(1-exp(-x(i)/e))-sin(pi*x(i)/2);
...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=exp(-t(j))*(c*exp(-x(i)/e)*(-x(i)/e)+x(i)*(1+x(i))*exp(x(i)-1)+sin(pi*x(i)/2)*((-e*(pi^2)/4)-(1+x(i)))+(pi/2)*cos(pi*x(i)/2));
        f2(i,j)=exp(-t(j))*(c*exp(-x(i)/e)*(-2*x(i)/e)+exp(x(i)-1)*((x(i)^2)+(1+2*x(i))*(1+e))+x(i)*sin(pi*x(i)/2));
end
end
%%%%%%%%%%%%%%%%%%%%
% for j=1:Nt+1
%     for i=1:Nx+1
%        U1_exct(i,j)=exp(-t(j))*(c*(1-exp(-x(i)/e))-sin(pi*x(i)/2));
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e*k/(h(i+1)*h(i)))+k*((1+x(i))/h(i+1))+k*(2+x(i))+1;
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e*k/(h(i+1)*h(i)))+k*((1+2*x(i))/h(i+1))+k*(2+(x(i)))+1;
   ...
   err1=abs(F1-G1(1:2:Nx+1,1:2:Nt+1));
    err_sup1=max(err1);
       ...
       error1(ep_counter,N_counter)=max_err1;
           ...
           %err2=abs(U2-U2_exct);
        err2=abs(F1-G1(1:2:Nx+1,1:2:Nt+1));
        err_sup2=max(err2);
