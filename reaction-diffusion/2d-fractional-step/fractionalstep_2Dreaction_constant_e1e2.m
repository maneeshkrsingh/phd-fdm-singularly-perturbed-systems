...
for l1=1:1
    ...
    Ny=32*2^(l1-1);
    Nt=8*2^(l1-1);
  sigx1=min(1/8,1*sqrt(e1)*log(Nx));
  sigx2=min(1/4,1*sqrt(e2)*log(Nx));
  sigy1=min(1/8,1*sqrt(e1)*log(Ny));
  sigy2=min(1/4,1*sqrt(e2)*log(Ny));
...
it;
    ...
    for j=1:Ny-1
      ...
      for i=2:Nx-2
        ...
        A1(i,i)=1+(2*e1*ht)/(hx(i)*hx(i+1))+ht; % b11=b11/2+b11/2 as defined for fractional step
     ...
     i=1;
        A1(i,i)=1+(2*e1*ht)/(hx(i)*hx(i+1))+ht; 
  ...
  %%%%%%%%%%%%%%%%%%%%%%   for second component equation %%%%%%%%%%%%%%%%%
    for i=2:Nx-2
        ...
        D1(i,i)=1+(2*e2*ht)/(hx(i)*hx(i+1))+ht;  % b22=b22/2+b22/2 as defined for fractional step
        %D1(i,i)=1+(2*e1*ht)/(hx(i)*hx(i+1))+ht*(1+t(k)*x(i)*y(j))/2;
        D1(i,i+1)=-(2*e2*ht)/((hx(i)+hx(i+1))*hx(i+1));
       
     end
     i=1;
        D1(i,i)=1+(2*e2*ht)/(hx(i)*hx(i+1))+ht;  
       %D1(i,i)=1+(2*e1*ht)/(hx(i)*hx(i+1))+ht*(1+t(k)*x(i)*y(j))/2;
        D1(i,i+1)=-(2*e2*ht)/((hx(i)+hx(i+1))*hx(i+1));
...
%%%%%%%%%%%%%%%%%%for second component in first equation %%%%%%%%%%%%%%%%%%%%
  for i=2:Nx-2
     B1(i,i-1)=0;
     B1(i,i)=-ht;%a12=b12/2+b12/2 as defined for fractional step
     B1(i,i+1)=0;
 end
 i=1;
     B1(i,i)=-ht;
...
%%%%%%%%%%%%%%%%%%for first component in second equation %%%%%%%%%%%%%%%%%%%%
 for i=2:Nx-2
     C1(i,i-1)=0;
     C1(i,i)=-ht/2;%b21=b21/2+a21/2 as defined for fractional step
...
%     for i=2:Nx-2
%            
%          b11(i)=ht*(exp(-t(k))*x(i)*(1-x(i))*y(j)*(1-y(j)))+u1(i+1,j,k-1);
%          b12(i)=ht*(exp(-t(k))*x(i)*(1-x(i)^2)*y(j)*(1-y(j)^2))+u2(i+1,j,k-1);
%     end
% 
%     
%          b11(1)=ht*(exp(-t(k))*x(1)*(1-x(1))*y(j)*(1-y(j)))+u1(2,j,k-1);
%          b12(1)=ht*(exp(-t(k))*x(1)*(1-x(1)^2)*y(j)*(1-y(j)^2))+u2(2,j,k-1);
%         
%          b11(Nx-1)=ht*(exp(-t(k))*x(Nx-1)*(1-x(Nx-1))*y(j)*(1-y(j)))+u1(Nx,j,k-1);
%          b12(Nx-1)=ht*(exp(-t(k))*x(Nx-1)*(1-x(Nx-1)^2)*y(j)*(1-y(j)^2))+u2(Nx,j,k-1);
   
    for i=2:Nx-2
           
         b11(i)=ht*1+u1(i+1,j,k-1);
         b12(i)=ht*1+u2(i+1,j,k-1);
    end

    
         b11(1)=ht*1+u1(2,j,k-1);
         b12(1)=ht*1+u2(2,j,k-1);
        
         b11(Nx-1)=ht*1+u1(Nx,j,k-1);
         b12(Nx-1)=ht*1+u2(Nx,j,k-1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
    for i=1:Nx-1
      for j=2:Ny-2
        ...
        A2(j,j)=1+(2*e1*ht)/(hy(j)*hy(j+1))+ht;   % b11=b11/2+a11/2 as defined for fractional step
       %A2(j,j)=1+(2*e1*ht)/(hy(j)*hy(j+1))+ht*(2+t(k)*x(i)*y(j)^2);
       A2(j,j+1)=-(2*e1*ht)/((hy(j)+hy(j+1))*hy(j+1));
       
     end
     j=1;
         A2(j,j)=1+(2*e1*ht)/(hy(j)*hy(j+1))+ht; 
        % A2(j,j)=1+(2*e1*ht)/(hy(j)*hy(j+1))+ht*(2+t(k)*x(i)*y(j)^2);
         A2(j,j+1)=-(2*e1*ht)/((hy(j)+hy(j+1))*hy(j+1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%for second component%%%%%%%%%%%%%%%%%%%%%%%%%%
 for j=2:Ny-2
        ...
        D2(j,j)=1+(2*e2*ht)/(hy(j)*hy(j+1))+ht;  % b22=b22/2+b22/2 as defined for fractional step
       %D2(j,j)=1+(2*e1*ht)/(hy(j)*hy(j+1))+ht*(1+t(k)*x(i)*y(j)^2);
       D2(j,j+1)=-(2*e2*ht)/((hy(j)+hy(j+1))*hy(j+1));
       
 end
     j=1;
        D2(j,j)=1+(2*e2*ht)/(hy(j)*hy(j+1))+ht; 
        %D2(j,j)=1+(2*e1*ht)/(hy(j)*hy(j+1))+ht*(1+t(k)*x(i)*y(j)^2);
       D2(j,j+1)=-(2*e2*ht)/((hy(j)+hy(j+1))*hy(j+1));
...
%          b22(Ny-1)=ht*(x(i)*(1-x(i))*y(Ny-1)*(1-y(Ny-1))*exp(-t(k)))+u2_temp(i,Ny,2*(k-1));
    ...
    for j=2:Ny-2
         ...
         b21(j)=ht*1+u1_temp(i,j+1,2*(k-1));
         b22(j)=ht*1+u2_temp(i,j+1,2*(k-1));
    end

         b21(1)=ht*1+u1_temp(i,2,2*(k-1));
         b22(1)=ht*1+u2_temp(i,2,2*(k-1));
         
         b21(Ny-1)=ht*1+u1_temp(i,Ny,2*(k-1));
         b22(Ny-1)=ht*1+u2_temp(i,Ny,2*(k-1));
           b2=[b21;b22];
