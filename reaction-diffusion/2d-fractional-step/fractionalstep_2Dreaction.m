...
for l1=1:3
    ...
    Ny1=8*2^(l1-1);
    Nt1=2*2^(l1-1);
  sigx=min(1/4,1*sqrt(e)*log(Nx1));
  sigy=min(1/4,2*sqrt(e)*log(Ny1));
...
it;
    ...
    for j=1:Ny-1
      ...
      for i=2:Nx-2
        ...
        A1(i,i-1)=-2*e*ht/((hx(i)+hx(i+1))*hx(i));
        A1(i,i)=1+(2*e*ht)/(hx(i)*hx(i+1))+ht*(4+2*t(k)*(x(i)^2)+2*t(k)*(y(j)^2))/2;  % b11=b11/2+b11/2 as defined for fractional step
        A1(i,i+1)=-(2*e*ht)/((hx(i)+hx(i+1))*hx(i+1));
       
     end
     i=1;
        A1(i,i)=1+(2*e*ht)/(hx(i)*hx(i+1))+ht*(4+2*t(k)*(x(i)^2)+2*t(k)*(y(j)^2))/2; 
        A1(i,i+1)=-(2*e*ht)/((hx(i)+hx(i+1))*hx(i+1));
  ...
  %%%%%%%%%%%%%%%%%%%%%%   for second component equation %%%%%%%%%%%%%%%%%
    for i=2:Nx-2
        ...
        D1(i,i-1)=-(2*e*ht)/((hx(i)+hx(i+1))*hx(i));
        D1(i,i)=1+(2*e*ht)/(hx(i)*hx(i+1))+ht*(2+4*t(k)*(x(i)^2)+(t(k)^2)*(y(j)^2))/2;  % b22=b22/2+b22/2 as defined for fractional step
        D1(i,i+1)=-(2*e*ht)/((hx(i)+hx(i+1))*hx(i+1));
       
     end
     i=1;
        D1(i,i)=1+(2*e*ht)/(hx(i)*hx(i+1))+ht*(2+4*t(k)*(x(i)^2)+(t(k)^2)*(y(j)^2))/2;  
        D1(i,i+1)=-(2*e*ht)/((hx(i)+hx(i+1))*hx(i+1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
    for i=1:Nx-1
      for j=2:Ny-2
        ...
        A2(j,j-1)=-(2*e*ht)/((hy(j)+hy(j+1))*hy(j));
        A2(j,j)=1+(2*e*ht)/(hy(j)*hy(j+1))+ht*(4+2*t(k)*(x(i)^2)+2*t(k)*(y(j)^2));  % b11=b11/2+a11/2 as defined for fractional step
        A2(j,j+1)=-(2*e*ht)/((hy(j)+hy(j+1))*hy(j+1));
       
     end
     j=1;
         A2(j,j)=1+(2*e*ht)/(hy(j)*hy(j+1))+ht*(4+2*t(k)*(x(i)^2)+2*t(k)*(y(j)^2));  
        A2(j,j+1)=-(2*e*ht)/((hy(j)+hy(j+1))*hy(j+1));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%for second component%%%%%%%%%%%%%%%%%%%%%%%%%%
 for j=2:Ny-2
        ...
        D2(j,j-1)=-(2*e*ht)/((hy(j)+hy(j+1))*hy(j));
        D2(j,j)=1+(2*e*ht)/(hy(j)*hy(j+1))+ht*(2+4*t(k)*(x(i)^2)+(t(k)^2)*(y(j)^2))/2;  % b22=b22/2+b22/2 as defined for fractional step
        D2(j,j+1)=-(2*e*ht)/((hy(j)+hy(j+1))*hy(j+1));
       
 end
     j=1;
        D2(j,j)=1+(2*e*ht)/(hy(j)*hy(j+1))+ht*(2+4*t(k)*(x(i)^2)+(t(k)^2)*(y(j)^2))/2; 
        D2(j,j+1)=-(2*e*ht)/((hy(j)+hy(j+1))*hy(j+1));
