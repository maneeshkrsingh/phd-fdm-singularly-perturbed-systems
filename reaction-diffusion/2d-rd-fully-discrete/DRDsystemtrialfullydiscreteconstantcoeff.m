...
for i=1:N+1
    for k=1:Nt+1
         ...
         u1(i,N+1,k)=0;        %%%%%%%%%%%%%%%% %u1(x,1,t)=0
         u2(i,N+1,k)=0;        %%%%%%%%%%%%%%%% %u2(x,1,t)=0
    end
end
...
%%%%%%%%%%%%%%%%%%%% %%%%%%%   matrix A     %%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=2:N
    ...
    for j=2:N
        ...
        if(i==2)
            if(k==1)
                A(k,k)=del_t*((2*e1/((bx+fx)*bx))+(2*e1/((bx+fx)*fx))+(2*e1/((by+fy)*by))+(2*e1/((by+fy)*fy))+5)+1;
        ...
        if(i~=2 &&  i~=N)
            if(k==1)             
             A((i-2)*(N-1)+k,(i-2)*(N-1)+k)=del_t*((2*e1/((bx+fx)*bx))+(2*e1/((bx+fx)*fx))+(2*e1/((by+fy)*by))+(2*e1/((by+fy)*fy))+5)+1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%   matrix D     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=2:N
    ...
    for j=2:N
        ...
        if(i==2)
            if(k==1)
                D(k,k)=del_t*((2*e2/((bx+fx)*bx))+(2*e2/((bx+fx)*fx))+(2*e2/((by+fy)*by))+(2*e2/((by+fy)*fy))+4)+1;
        ...
        if(i~=2 &&  i~=N)
            if(k==1)             
             D((i-2)*(N-1)+k,(i-2)*(N-1)+k)=del_t*((2*e2/((bx+fx)*bx))+(2*e2/((bx+fx)*fx))+(2*e2/((by+fy)*by))+(2*e2/((by+fy)*fy))+4)+1;
  ...
  for i=2:N
    ...
    for j=2:N
        ...
        if(i~=2 &&  i~=N)
            if(k==1)             
             C1((i-2)*(N-1)+k,(i-2)*(N-1)+k)=-del_t*(3);
  ...
  for i=2:N
    ...
    for j=2:N
        ...
        if(i~=2 &&  i~=N)
            if(k==1)             
             C2((i-2)*(N-1)+k,(i-2)*(N-1)+k)=-del_t*(1);
...
%    % l=0;
  for i=2:N
    ...
    %    l=l+1;
        if(i==2)
            if(k==1)
                B1(k)=del_t*(1-x(i)*y(j))*(x(i)*y(j))+u1(i,j,n);         %BCs are zero
            end
            if(k~=1 && k~=N-1)
                B1(k)= del_t*(1-x(i)*y(j))*(x(i)*y(j))+u1(i,j,n);       %BCs are zero
            end
        ...
        if(i~=2 &&  i~=N)
            if(k==1)
                B1((i-2)*(N-1)+k)=del_t*(1-x(i)*y(j))*(x(i)*y(j))+u1(i,j,n);           %BCs are zero
            end
            if(k~=1 && k~=N-1)
                 B1((i-2)*(N-1)+k)=del_t*(1-x(i)*y(j))*(x(i)*y(j))+u1(i,j,n);  
            end
            if(k==N-1)
                B1((i-2)*(N-1)+k)=del_t*(1-x(i)*y(j))*(x(i)*y(j))+u1(i,j,n);          %BCs are zero
            end  
   ...
   for i=2:N
    ...
    %    l=l+1;
        if(i==2)
            if(k==1)
                B2(k)=del_t*(1-(x(i)^2)*(y(j)^2))*(x(i)^2)*(y(j)^2)+u2(i,j,n);         %BCs are zero
            end
            if(k~=1 && k~=N-1)
                B2(k)=del_t*(1-(x(i)^2)*(y(j)^2))*(x(i)^2)*(y(j)^2)+u2(i,j,n);          %BCs are zero
            end
        ...
        if(i~=2 &&  i~=N)
            if(k==1)
                B2((i-2)*(N-1)+k)=del_t*(1-(x(i)^2)*(y(j)^2))*(x(i)^2)*(y(j)^2)+u2(i,j,n);            %BCs are zero
            end
            if(k~=1 && k~=N-1)
                 B2((i-2)*(N-1)+k)=del_t*(1-(x(i)^2)*(y(j)^2))*(x(i)^2)*(y(j)^2)+u2(i,j,n);   
            end
            if(k==N-1)
                B2((i-2)*(N-1)+k)=del_t*(1-(x(i)^2)*(y(j)^2))*(x(i)^2)*(y(j)^2)+u2(i,j,n);            %BCs are zero
            end  
