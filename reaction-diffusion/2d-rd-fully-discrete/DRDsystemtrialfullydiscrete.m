...
%%%%%%%%%%%%%%%%%%%% %%%%%%%   matrix A     %%%%%%%%%%%%% %%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=2:N
    ...
    for j=2:N
        ...
        if(i==2)
            if(k==1)
                A(k,k)=del_t*((2*e1/((bx+fx)*bx))+(2*e1/((bx+fx)*fx))+(2*e1/((by+fy)*by))+(2*e1/((by+fy)*fy))+2+(x(i)^2)*(y(j)^2))+1;
        ...
        if(i~=2 &&  i~=N)
            if(k==1)             
             A((i-2)*(N-1)+k,(i-2)*(N-1)+k)=del_t*((2*e1/((bx+fx)*bx))+(2*e1/((bx+fx)*fx))+(2*e1/((by+fy)*by))+(2*e1/((by+fy)*fy))+2+(x(i)^2)*(y(j)^2))+1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%   matrix D     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=2:N
    ...
    for j=2:N
        ...
        if(i==2)
            if(k==1)
                D(k,k)=del_t*((2*e2/((bx+fx)*bx))+(2*e2/((bx+fx)*fx))+(2*e1/((by+fy)*by))+(2*e2/((by+fy)*fy))+4+(x(i)^2)*(y(j)^2))+1;
        ...
        if(i~=2 &&  i~=N)
            if(k==1)             
             D((i-2)*(N-1)+k,(i-2)*(N-1)+k)=del_t*((2*e2/((bx+fx)*bx))+(2*e2/((bx+fx)*fx))+(2*e2/((by+fy)*by))+(2*e2/((by+fy)*fy))+4+(x(i)^2)*(y(j)^2))+1;
