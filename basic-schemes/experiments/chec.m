...
for j=1:Nt+1
    for i=1:Nx+1
        f(i,j)=10*(t(j)^2)*exp(-t(j))*x(i)*(1-x(i));
    end
end
...
for i=2:Nx/2
    A(i,i-1)= -k*((2*e/((h(i))*(h(i+1)+h(i))))+((a(i+1)+a(i)))/2*(h(i-1))+((b(i)+b(i+1))/2))+0.5;
   A(i,i)=k*((2*e/(h(i+1)*h(i)))+((a(i)+a(i+1))/2*(h(i)))+((b(i)+b(i+1))/2))+0.5;
    A(i,i+1)=-k*(2*e/(h(i+1)*(h(i+1)+h(i))));
end

for i=(Nx/2)+1:Nx-2
     A(i,i-1)=-k*((2*e/(h(i)*(h(i+1)+h(i))))+(a(i-1)/(h(i)+h(i+1))));
    A(i,i)=k*((2*e/(h(i)*h(i+1)))+b(i))+1;
    A(i,i+1)=-k*((2*e/(h(i)*(h(i)+h(i+1))))-(a(i-1)/(h(i)+h(i+1))));
end
i=2;
    A(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+((a(i)+a(i-1))/2*(h(i)))+((b(i)+b(i-1))/2))+0.5;
