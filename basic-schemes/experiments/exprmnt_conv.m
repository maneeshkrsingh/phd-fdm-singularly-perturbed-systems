...
for N_counter =1:2
 ...
 Nx1;
 Nt1;
sig2=min(1/4,2*sqrt(e2)*log(Nx1));
sig1=min(sig2/2,2*sqrt(e1)*log(Nx1));
sig2;
sig1;
 for l=1:2
     Nx=Nx1*2^(l-1);
...
k;
%%%%%%%%%%%%%%%%%%%
% numerical approximation of initial and boundary value conditions and nonfomogeneous term
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*(3+(x(i)*t(j+1)))+1;
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*(3+(x(i))^2)+1;
...
% %    for j=2:Nt  
     for i=2:Nx-2
             b1(i)=(k*f1(i,j+1)+U1(i+1,j));
              b2(i)=(k*f2(i,j+1)+U2(i+1,j));
     end
     
      b1(1)=(k*f1(1,j+1)+U1(2,j));
      b2(1)=(k*f2(1,j+1)+U2(2,j));
      b1(Nx-1)=k*f1(Nx-1,j+1)+U1(Nx,j);
      b2(Nx-1)=k*f2(Nx-1,j+1)+U2(Nx,j);
      b=[b1;b2];
