...
for j=1:Nt+1
    for i=1:Nx+1
      ...
      %  f2(i,j)=exp(-4*t(j));
       ...
       f2(i,j)=((x(i)^2)-2*x(i)+1)*exp(-4*t(j));
       % f1(i,j)=(1-2*(x(i))-(x(i)^2))*exp(-t(j)^3);
       % f2(i,j)=(1-(x(i))-(x(i)^3))*exp(-t(j));
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k/h(i+1)+k*2+1;
 ...
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*2/h(i+1)+k*4+1;
     D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*2/h(i+1);
 end
 i=1;
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*2/h(i+1)+k*4+1;
     D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*2/h(i+1);
