...
sig2=min(1/2,5.2*e2*log(Nx1));
sig1=min(sig2/2,5.2*(e1)*log(Nx1));
% sig2=0.0024;
% sig1=0.0014;
for l=1:2
     Nx=Nx1*2^(l-1);
...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=((x(i)^2)-x(i)+1)*(t(j))*exp(-t(j));
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 for j=2:Nt+1
for i=3:Nx/2+1
    A(i-1,i-2)=k*(-(2*e1/(h(i-1)*(h(i-1)+h(i))))-(((1+x(i))+(1+x(i-1)))/(2*h(i-1)))+((2+x(i)*t(j))+(2+x(i-1)*t(j)))/2)+(1/2);
    A(i-1,i-1)=k*((2*e1/(h(i-1)*h(i)))+(((1+x(i))+(1+x(i-1)))/(2*h(i-1)))+((2+x(i)*t(j))+(2+x(i-1)*t(j)))/2)+(1/2);
    A(i-1,i)=-k*(2*e1/(h(i)*(h(i-1)+h(i))));
end
for i=(Nx/2)+2:Nx-1
    A(i-1,i-2)=k*(-(2*e1/(h(i-1)*(h(i-1)+h(i))))-((1+x(i))/(h(i-1)+h(i))));
    A(i-1,i-1)=k*((2*e1/(h(i-1)*h(i)))+(2+x(i)*t(j)))+1;
...
i=2;
 ...
 for i=3:Nx/2+1
    D(i-1,i-2)=k*(-(2*e2/(h(i-1)*(h(i-1)+h(i))))-(((1+2*x(i))+(1+2*x(i-1)))/(2*h(i-1)))+((2+2*x(i)*t(j))+(2+2*x(i-1)*t(j)))/2)+(1/2);
    D(i-1,i-1)=k*((2*e2/(h(i-1)*h(i)))+(((1+2*x(i))+(1+2*x(i-1)))/(2*h(i-1)))+((2+2*x(i)*t(j))+(2+2*x(i-1)*t(j)))/2)+(1/2);
    D(i-1,i)=-k*(2*e2/(h(i)*(h(i-1)+h(i))));
end
for i=(Nx/2)+2:Nx-1
    D(i-1,i-2)=k*(-(2*e2/(h(i-1)*(h(i-1)+h(i))))-((1+2*x(i))/(h(i-1)+h(i))));
    D(i-1,i-1)=k*((2*e2/(h(i-1)*h(i)))+(2+2*x(i)*t(j)))+1;
...
i=2;
 ...
 i=Nx;   
  ...
  for i=3:Nx/2+1
     B(i-1,i-2)=-k*(x(i)*t(j)+x(i-1)*t(j))/4;
     B(i-1,i-1)=-k*(x(i)*t(j)+x(i-1)*t(j))/4;
     B(i-1,i)=0;
  end
 for i=(Nx/2)+2:Nx-1
     B(i-1,i-2)=0;
