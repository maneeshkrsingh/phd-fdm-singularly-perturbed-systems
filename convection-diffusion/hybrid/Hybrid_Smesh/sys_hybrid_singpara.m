...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for j=2:Nt+1
 for i=3:Nx/2-1
      A(i-1,i-2)=k*(-2*e/(h(i-1)*(h(i-1)+h(i)))+(1+x(i))/(h(i-1)+h(i)));
      A(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+(1+2*x(i)))+1;
      A(i-1,i)=k*((-2*e/(h(i)*(h(i-1)+h(i))))-(1+x(i))/(h(i-1)+h(i)));
 end
 
 for i=(Nx/2):Nx-1
    A(i-1,i-2)=k*(-2*e/(h(i-1)*(h(i-1)+h(i)))+((1+2*x(i))+(1+2*x(i-1)))/4);
    A(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+((1+2*x(i))+(1+2*x(i-1)))/4+((1+x(i))+(1+x(i-1)))/(2*h(i)));
    A(i-1,i)=-k*(2*e/(h(i)*(h(i-1)+h(i)))+((1+x(i))+(1+x(i-1)))/(2*h(i)));
end   
 i=2;
      A(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+(1+2*x(i)))+1;
      A(i-1,i)=k*((-2*e/(h(i)*(h(i-1)+h(i))))-(1+x(i))/(h(i-1)+h(i)));
...
%D
for i=3:Nx/2-1
      D(i-1,i-2)=k*(-2*e/(h(i-1)*(h(i-1)+h(i)))+(1+2*x(i))/(h(i-1)+h(i)));
      D(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+(1+x(i)))+1;
      D(i-1,i)=k*((-2*e/(h(i)*(h(i-1)+h(i))))-(1+2*x(i))/(h(i-1)+h(i)));
 end
 
 for i=(Nx/2):Nx-1
    D(i-1,i-2)=k*(-2*e/(h(i-1)*(h(i-1)+h(i)))+((1+x(i))+(1+x(i-1)))/4)+1/2;
    D(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+((1+x(i))+(1+x(i-1)))/4+((1+2*x(i))+(1+2*x(i-1)))/(2*h(i)))+1/2;
    D(i-1,i)=-k*(2*e/(h(i)*(h(i-1)+h(i)))+((1+2*x(i))+(1+2*x(i-1)))/(2*h(i)));
end   
 i=2;
      D(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+(1+x(i)))+1;
      D(i-1,i)=k*((-2*e/(h(i)*(h(i-1)+h(i))))-(1+2*x(i))/(h(i-1)+h(i)));
...
% % %    for j=2:Nt  
     for i=3:(Nx/2)-1
             b1(i-1)=(k*f1(i,j)+U1(i+1,j-1));
              b2(i-1)=(k*f2(i,j)+U2(i+1,j-1));
     end
     for i=(Nx/2):Nx
