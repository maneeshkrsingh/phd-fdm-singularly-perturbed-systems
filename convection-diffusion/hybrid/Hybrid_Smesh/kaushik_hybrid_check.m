...
for i=(Nx/2)+1:Nx-1
    A(i-1,i-2)=k*(-(2*e/(h(i-1)*(h(i-1)+h(i))))-((1+x(i)*(1-x(i)))/(h(i-1)+h(i))));
    A(i-1,i-1)=k*((2*e/(h(i-1)*h(i))))+1;
    A(i-1,i)=k*(-(2*e/(h(i)*(h(i-1)+h(i))))+( (1+x(i)*(1-x(i)))/(h(i-1)+h(i))));
end
i=2;
   A(i-1,i-1)=k*((2*e/(h(i-1)*h(i)))+(((1+x(i-1)*(1-x(i-1)))+(1+x(i)*(1-x(i))))/(2*h(i-1))))+(1/2);
 ...
 %%%%%%%%%%%%%%%%%%%%%
    for i=3:(Nx/2)
     ...
     for i=(Nx/2)+1:Nx
      ...
      d(1)=0.5*k*(f(1,j)+f(2,j))+0.5*(U(2,j-1)+U(1,j-1));
   d(Nx-1)=k*f(Nx,j)+U(Nx,j-1);
       A ; 
      D=A\d;
      %%%%%%%%%%%%%%%%%%%%%%%%% numerical solution
       for i=2:Nx
