...
for j=1:Nt+1
   for i=1:Nx+1
       ...
       % f2(i,j)=exp(-4*t(j));
       f1(i,j)=(1+x(i))*U1_old(i,j)+(U1_old(i,j)^2)+exp(U1_old(i,j)-U2_old(i,j));
       f2(i,j)=(1+2*x(i))*U2_old(i,j)+(U2_old(i,j)^3)+exp(U2_old(i,j)-U1_old(i,j));
   end
end
%%%%%%%%%%%%%%%%%%
% for j=1:Nt+1
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k/h(i+1)+k*((1+x(i))+2*U1_old(i,j)+exp(U1_old(i,j)-U2_old(i,j)))+1;
...
%      A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k/h(i+1);
 ...
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k/h(i+1)++k*((1+2*x(i))+3*(U2_old(i,j)^2)+exp(U2_old(i,j)-U1_old(i,j)))+1;
...
%end
   ...
   if(max(max(abs(U1_old-F1)))<0.0001)   
       flag=0;
   end
...
%      
     ...
     %  end
       %%%%%%%%%%%%%%%%checking tolernce first componenet %%%%%%%%%%%%%% 
   if(max(max(abs(U1_old-G1)))<0.0001)   
       flag=0;
   end
    %%%%%%%%%%%%%Agian iteration %%%%%%%%%%%%%
    U1_old=G1;
     ...
     %%%%%%%%%%%%%%%%checking tolernce second componenet %%%%%%%%%%%%%%
   if(max(max(abs(U2_old-G2)))<0.0001)   
       flag=0;
   end 
    %%%%%%%%%%%%%Agian iteration %%%%%%%%%%%%%
   U2_old=G2;
