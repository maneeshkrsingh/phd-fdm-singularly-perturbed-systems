...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*(1+x(i)*t(j))/h(i+1)+k*(2+x(i)*t(j))+1;
     A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k*(1+x(i)*t(j))/h(i+1);
 ...
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*(1+2*x(i)*t(j))/h(i+1)+k*(2+2*x(i)*t(j))+1;
      D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*(1+2*x(i)*t(j))/h(i+1);
