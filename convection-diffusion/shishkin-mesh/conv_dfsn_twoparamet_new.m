...
% nonfomogeneous term and exact solution
 ...
 for i=1:Nx+1
   % U1(i,1)=(m1+m2*(1-x(i))-exp(-(x(i))/e2));
    U1(i,1)=(c1*(1-exp(-(x(i))/e1))+c2*(1-exp(-(x(i))/e2))-2*(x(i)));
    U2(i,1)=(m1+m2*(1-x(i))-exp(-x(i)/e2));
end
 for j=1:Nt
    U1(1,j)=0;
    U1(Nx+1,j)=0;
...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=exp(-t(j))*(exp(-(x(i))/e1)*(c1-(x(i)/e1)*c1)+(1-x(i))*exp(-(x(i))/e1)*(1-(1+x(i))*(c2/e2)+(c2*e1/(e2)^2)-c2)+(-m1-m2*(1-x(i))+2+c1+c2));
       ...
       %f2(i,j)=exp(-t(j))*(exp(-(1-x(i))/e2)*(-(x(i)/e2)-2*x(i))+(2*c1*x(i)+c2*(x(i)+2*(x(i)^2)+1))-(m1+m2*x(i)-exp(-(1-x(i))/e1)));
         f2(i,j)=exp(-t(j))*(exp(-(x(i))/e1)*c1+exp(-(x(i))/e2)*(c2-2*(x(i)/e2)-1)+(-c1-c2+2*x(i)+m1+m2*(1-x(i))+m2*(1+2*x(i))));
end
end
%%%%%%%%%%%%%%%%%%
for j=1:Nt+1
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*((1+x(i))/h(i+1))+2*k+1;
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*((1+2*x(i))/h(i+1))+2*k+1;
