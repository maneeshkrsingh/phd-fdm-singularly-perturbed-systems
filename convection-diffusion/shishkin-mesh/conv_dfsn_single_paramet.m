...
% nonfomogeneous term and exact solution
 ...
 for i=1:Nx+1
    ...
    U2(i,1)=(c1*(1-exp(-(x(i))/e))-(x(i))*exp(x(i)-1));
...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=exp(-t(j))*(exp(-(x(i))/e)*(-(x(i)/e)-2*x(i)+c1)+(x(i))*exp(x(i)-1)+(2*m1*x(i)+2*m2*(x(i)^2)-m1-m2*x(i)-c1));
       f2(i,j)=exp(-t(j))*(exp(-(x(i))/e)*(-c1*x(i)+x(i)-(2*x(i)/e))+exp(x(i)-1)*(1+3*x(i)+(x(i)^2)+(2*x(i)*e)+e+c1*(x(i))));
end
end
%%%%%%%%%%%%%%%%%%
for j=1:Nt+1
    for i=1:Nx+1
       ...
       U2_exct(i,j)=exp(-t(j))*(c1*(1-exp(-(x(i))/e))-(x(i))*exp(x(i)-1));
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e*k/(h(i+1)*h(i)))+k*((1+x(i))/h(i))+k*(1+2*x(i))+1;
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e*k/(h(i+1)*h(i)))+k*((1+2*x(i))/h(i))+k*(1+(x(i)))+1;
