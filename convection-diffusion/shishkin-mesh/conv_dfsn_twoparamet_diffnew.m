...
sig2=min(1/2,e2*log(Nx));
sig1=min(1/4,e1*log(Nx));
sig2;
sig1;
% for l=1:2
%      Nx=Nx*2^(l-1);
...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=exp(-t(j))*(exp(-(x(i))/e1)*(-1-(x(i)/e1))+exp(-(x(i))/e2)+(c1+c2)-(m1+m2)+m2*x(i));
       ...
       %f2(i,j)=exp(-t(j))*(exp(-(1-x(i))/e2)*(-(x(i)/e2)-2*x(i))+(2*c1*x(i)+c2*(x(i)+2*(x(i)^2)+1))-(m1+m2*x(i)-exp(-(1-x(i))/e1)));
         f2(i,j)=exp(-t(j))*(exp(-(x(i))/e2)*(-1-(x(i)/e2))+exp(-(x(i))/e1)+(m1+m2)-(c1+c2)+c2*x(i));
end
end
%%%%%%%%%%%%%%%%%%
for j=1:Nt+1
    for i=1:Nx+1
       U1_exct(i,j)=exp(-t(j))*(c1+c2*(1-x(i))-exp(-x(i)/e1));
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*((1+x(i))/h(i+1))+2*k+1;
     D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*((1+x(i))/h(i+1));
 end
 i=1;
       D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*((1+x(i))/h(i+1))+2*k+1;
     D(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*((1+x(i))/h(i+1));
