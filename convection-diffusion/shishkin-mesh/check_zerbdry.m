...
for j=1:Nt+1
    for i=1:Nx+1
        ...
        f1(i,j)=exp(-t(j))*((exp(-x(i)/e1)*((-2*(x(i)/e1)-x(i))*c1))+(exp(-x(i)/e2)*(c2*(-x(i)-((1+2*x(i))/e2)+(e1/(e2^2)))+1))+(x(i)*(c1+c2-2*x(i))+2*(1+2*x(i))-m1-m2*(1-x(i))));
        f2(i,j)=exp(-t(j))*((exp(-x(i)/e1)*c1)+(exp(-x(i)/e2)*(-(x(i)/e2)-2*x(i)+c2))+(2*m1*x(i)+m2*(3*x(i)+1-2*(x(i)^2))+2*x(i)-(c1+c2)));
    end
end
%%%%%%%%%%%%%%%%%%
for j=1:Nt+1
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for i=2:Nx-2
     ...
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*((1+2*x(i))/h(i+1))+k*(1+x(i))+1;
     A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k*(1+2*x(i))/h(i+1);
 end
 i=1;
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*((1+2*x(i))/h(i+1))+k*(1+x(i))+1;
     A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k*(1+2*x(i))/h(i+1);
...
%D
 for i=2:Nx-2
     ...
     D(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*((1+x(i))/h(i+1))+k*(1+2*(x(i)))+1;
