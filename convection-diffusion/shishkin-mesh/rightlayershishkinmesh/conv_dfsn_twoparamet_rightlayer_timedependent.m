...
sig1=min(sig2/2,3.2*(e1)*log(Nx1));
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 for j=2:Nt+1
for i=2:Nx-2
    A(i,i-1)=k*(-(2*e1/(h(i)*(h(i+1)+h(i))))-(1+x(i))/h(i));
    A(i,i)=k*((2*e1/(h(i+1)*h(i)))+((1+x(i))/h(i))+(2+x(i)*t(j)))+1;
    A(i,i+1)=-k*(2*e1/(h(i+1)*(h(i+1)+h(i))));
end
...
i=1;
 ...
 for i=2:Nx-2
    D(i,i-1)=k*(-(2*e2/(h(i)*(h(i+1)+h(i))))-(1+x(i))/h(i));
    D(i,i)=k*((2*e2/(h(i+1)*h(i)))+((1+x(i))/h(i))+(2+2*x(i)*t(j)))+1;
    D(i,i+1)=-k*(2*e2/(h(i+1)*(h(i+1)+h(i))));
end
