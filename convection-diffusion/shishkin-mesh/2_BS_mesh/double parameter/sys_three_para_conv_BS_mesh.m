clc
clear all
format short e

%for ep_counter=1:10
    clear A
    clear B
    clear C
    clear D
%     e2=10^((ep_counter-1));
%     e1=10^((ep_counter));
%  e1=2^(-20);
%  e2=2^(-12);
%  e3=2^(-11);
 e1=2^(-20);
 e2=2^(-11);
 e3=2^(-10);


%     e3=2^(-2*(ep_counter+2));
%     e2=2^(-2*(ep_counter+4));
%     e1=2^(-2*(ep_counter+6));
    %e2=2^(-2*(ep_counter+1));
    %e1=e2/2;
    %m1=exp(-1/e2);
    %m2=1-exp(-1/e2);
    c1=1/(1-exp(-1/e1));
    c2=1/(1-exp(-1/e2));
    c3=1/(1-exp(-1/e3));

for N_counter =1:5
    Nx1=3*8*2^(N_counter-1);
    Nt1=Nx1/4;
sig3=min(1/2,2*e3*log(Nx1));
sig2=min(sig3/2,2*e2*log(Nx1));
sig1=min(sig2/2,2*(e1)*log(Nx1));
% sig2=0.0024;
% sig1=0.0014;
for l=1:2
     Nx=Nx1*2^(l-1);
    Nt=Nt1*2^(l-1);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%U=zeors(2*Nx+2,Nt+1);
% U_num=zeros(2*Nx-2,Nt+1);
U1=zeros(Nx+1,Nt+1); %U1(2:Nx,:)=U_num(1:Nx-1,:);
%U_1=zeros(Nx-1,Nt);
U2=zeros(Nx+1,Nt+1); %U2(2:Nx,:)=U_num(Nx:2*Nx-2,:); 
U3=zeros(Nx+1,Nt+1); %U2(2:Nx,:)=U_num(Nx:2*Nx-2,:); 
%U_2=zeros(Nx-1,Nt);
%U1_exct=zeros(Nx+1,Nt+1);
%U2_exct=zeros(Nx+1,Nt+1);
f1=zeros(Nx-1,1);
f2=zeros(Nx-1,1);
f3=zeros(Nx-1,1);
A=zeros(Nx-1);
B=zeros(Nx-1);
C=zeros(Nx-1);
D=zeros(Nx-1);
E=zeros(Nx-1);
F=zeros(Nx-1);
G=zeros(Nx-1);
H=zeros(Nx-1);
I=zeros(Nx-1);
b1=zeros(Nx-1,1);
b2=zeros(Nx-1,1);
b3=zeros(Nx-1,1);
d1=zeros(Nx-1,1);
d2=zeros(Nx-1,1);
d3=zeros(Nx-1,1);
b=zeros(3*Nx-3,1);
P=zeros(3*Nx-3,1);
M=zeros(3*Nx-3,3*Nx-3);
x=zeros(Nx+1,1);
h=zeros(Nx,1);
t=zeros(Nt+1,1);
%c=zeros(Nx-1,1);
%%%%%%%%%%%%%%%%%%%%%%%


% construction of mesh points along  spatial variable and time variable
x(1)=0;
for i=1:(Nx/6)
    x(i+1)=-2*e1*log(1-6*(1-(1/Nx))*(i/Nx));
end
for i=(Nx/6)+1:(2*Nx/6)
    x(i+1)=x((Nx/6)+1)-2*(e2-e1)*log(1-6*(1-(1/Nx))*(i/Nx-1/6));
    %x(i+1)=x((N/4)+1)+i*(4*((sig2)-(sig1)))/N;
end

for i=(2*Nx/6)+1:(Nx/2)
   x(i+1)=x((2*Nx/6)+1)-2*(e3-e2)*log(1-6*(1-(1/Nx))*(i/Nx-1/3));
end

for i=(Nx/2)+1:Nx-1
   x(i+1)=1-((1-2*e3*log(Nx))*2*(Nx-i)/Nx);
end
x(Nx+1)=1;
for i=1:Nx
h(i)=x(i+1)-x(i);
end

% for i=1:(Nx/6)
%       h(i)=(6*sig1)/Nx;
% end
% 
% for i=(Nx/6)+1:2*Nx/6
%     h(i)=(6*((sig2)-(sig1)))/Nx;
% end
%  for i=(2*Nx/6)+1:Nx/2
%     h(i)=(6*((sig3)-(sig2)))/Nx;
%  end
% for i=(Nx/2)+1:Nx
%     h(i)=(2*(1-sig3))/Nx;
% end


 %%%%%%%%%%
%x;
for j=1:Nt+1
    t(j)=((j-1))/Nt;
end
k=1/Nt;
t;
k;
%%%%%%%%%%%%%%%%%%%
% numerical approximation of initial and boundary value conditions and
% nonfomogeneous term and exact solution

 for i=1:Nx+1
     U1(i,1)=0;
     U2(i,1)=0;
     U3(i,1)=0;
%     U1(i,1)=(c1*(1-exp(-x(i)/e1))-x(i));
%      U2(i,1)=(c2*(1-exp(-x(i)/e2))-x(i));
%     U3(i,1)=(c3*(1-exp(-x(i)/e3))-x(i));
end
 for j=1:Nt
    U1(1,j)=0;
    U1(Nx+1,j)=0;
    
    U2(1,j)=0;
    U2(Nx+1,j)=0;
    
     U3(1,j)=0;
    U3(Nx+1,j)=0;
    
 end
for j=1:Nt+1
    for i=1:Nx+1
     f1(i,j)=1;
        f2(i,j)=1;
        f3(i,j)=1;
        
%         f1(i,j)=((x(i)^3)-x(i))*exp(-t(j));
%         f2(i,j)=((x(i)^4)-x(i))*exp(-4*t(j));
%         f3(i,j)=((x(i)^2)-x(i))*exp(-t(j));
    end
end
%%%%%%%%%%%%%%%%%%
% for j=1:Nt+1
%     for i=1:Nx+1
%        %U2_exct(i,j)=exp(-t(j))*(m1+m2*(1-x(i))-exp(-x(i)/e2));
%        U1_exct(i,j)=exp(-t(j))*(c1*(1-exp(-x(i)/e1))+c2*(1-exp(-x(i)/e2))-2*x(i));
%        U2_exct(i,j)=exp(-t(j))*(m1+m2*(1-x(i))-exp(-x(i)/e2));
%     end
% end
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 % A
 for j=2:Nt+1
 for i=2:Nx-2
     A(i,i-1)=-2*e1*k/((h(i+1)+h(i))*h(i));
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*(1+x(i))/h(i+1)+k*(2+x(i))+1;
     A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k*(1+x(i))/h(i+1);
 end
 i=1;
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*(1+x(i))/h(i+1)+k*(2+x(i))+1;
     A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k*(1+x(i))/h(i+1);
     
 i=Nx-1;   
   A(i,i-1)=-2*e1*k/((h(i+1)+h(i))*h(i));
     A(i,i)=(2*e1*k/(h(i+1)*h(i)))+k*(1+x(i))/h(i+1)+k*(2+x(i))+1;
     
 for i=2:Nx-2
     E(i,i-1)=-2*e2*k/((h(i+1)+h(i))*h(i));
     E(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*(1+2*x(i))/h(i+1)+k*(2+2*(x(i)))+1;
     E(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*(1+2*x(i))/h(i+1);
 end
 i=1;
     E(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*(1+2*x(i))/h(i+1)+k*(2+2*(x(i)))+1;
     E(i,i+1)=(-2*e2*k/((h(i+1)+h(i))*h(i+1)))-k*(1+2*x(i))/h(i+1);
     
 i=Nx-1;   
       E(i,i-1)=-2*e2*k/((h(i+1)+h(i))*h(i));
     E(i,i)=(2*e2*k/(h(i+1)*h(i)))+k*(1+2*x(i))/h(i+1)+k*(2+2*(x(i)))+1;
     
 for i=2:Nx-2
     I(i,i-1)=-2*e3*k/((h(i+1)+h(i))*h(i));
     I(i,i)=(2*e3*k/(h(i+1)*h(i)))+k*(1+3*x(i))/h(i+1)+k*(3+2*(x(i)))+1;
     I(i,i+1)=(-2*e3*k/((h(i+1)+h(i))*h(i+1)))-k*(1+3*x(i))/h(i+1);
 end
 i=1;
     I(i,i)=(2*e3*k/(h(i+1)*h(i)))+k*(1+3*x(i))/h(i+1)+k*(3+2*(x(i)))+1;
     I(i,i+1)=(-2*e3*k/((h(i+1)+h(i))*h(i+1)))-k*(1+3*x(i))/h(i+1);
     
 i=Nx-1;   
      I(i,i-1)=-2*e3*k/((h(i+1)+h(i))*h(i));
     I(i,i)=(2*e3*k/(h(i+1)*h(i)))+k*(1+3*x(i))/h(i+1)+k*(3+2*(x(i)))+1;  

     
  for i=2:Nx-2
     B(i,i-1)=0;
     B(i,i)=-k;
     B(i,i+1)=0;
 end
 i=1;
     B(i,i)=-k;
     B(i,i+1)=0;
     
 i=Nx-1;   
     B(i,i-1)=0;
     B(i,i)=-k;
     
%    for i=2:Nx-2
%      C(i,i-1)=0;
%      C(i,i)=-k;
%      C(i,i+1)=0;
%  end
%  i=1;
%      C(i,i)=-k;
%      C(i,i+1)=0;
%      
%  i=Nx-1;   
%      C(i,i-1)=0;
%      C(i,i)=-k;  
     

   M=[A,B,B;B,E,B;B,B,I];
  
   

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %    for j=2:Nt  
     for i=2:Nx-2
             b1(i)=(k*f1(i,j)+U1(i+1,j-1));
              b2(i)=(k*f2(i,j)+U2(i+1,j-1));
              b3(i)=(k*f3(i,j)+U3(i+1,j-1));
     end
%      
      b1(1)=(k*f1(1,j)+U1(2,j-1));
      b2(1)=(k*f2(1,j)+U2(2,j-1));
      b3(1)=(k*f3(1,j)+U3(2,j-1));
      b1(Nx-1)=k*f1(Nx-1,j)+U1(Nx,j-1);
      b2(Nx-1)=k*f2(Nx-1,j)+U2(Nx,j-1);
      b3(Nx-1)=k*f3(Nx-1,j)+U3(Nx,j-1);
      b=[b1;b2;b3];
     
      P=inv(M)*b;
       %%%%%%%%%%%%%%%%%%%%%%%% numerical solution

      for i=2:(Nx)
          U1(i,j)=P(i-1);
      end
   
      for i=Nx+1:2*(Nx)-1
          U2(i-Nx,j)=P(i-1);
      end
      
      for i=2*(Nx):3*(Nx)-2
          U3(i-(2*(Nx)-1),j)=P(i-1);
      end
      
 end
%end
     if(l==1)
          F1=U1;
          F2=U2;
          F3=U3;
    end
       if(l==2)
          G1=U1;
          G2=U2;
          G3=U3;
       end
%        
end
  %%%%%%%%%%%%%%%%%%%%%
 
  % err1=abs(U1-U1_exct);
  err1=abs(F1-G1(1:2:Nx+1,1:2:Nt+1));
    err_sup1=max(err1);
         max_err1=max(err_sup1);
      %error1(ep_counter,N_counter)=max_err1;
       error1(N_counter)=max_err1;
          
           %err2=abs(U2-U2_exct);
        err2=abs(F2-G2(1:2:Nx+1,1:2:Nt+1));
        err_sup2=max(err2);
         max_err2=max(err_sup2);
       %error2(ep_counter,N_counter)=max_err2;
       error2(N_counter)=max_err2;
       
       err3=abs(F3-G3(1:2:Nx+1,1:2:Nt+1));
        err_sup3=max(err3);
         max_err3=max(err_sup3);
       %error3(ep_counter,N_counter)=max_err3;
       error3(N_counter)=max_err3;

 
end
%end
%end
%for j=1:ep_counter
for i=1:N_counter-1
         %cnv_rt1(j,i)=log2(error1(j,i)/error1(j,i+1));
         cnv_rt1(i)=log2(error1(i)/error1(i+1));
end
%end
%for j=1:ep_counter
 for i=1:N_counter-1
         %cnv_rt2(j,i)=log2(error2(j,i)/error2(j,i+1));
        cnv_rt2(i)=log2(error2(i)/error2(i+1));
 end
%end

%for j=1:ep_counter
 for i=1:N_counter-1
         %cnv_rt3(j,i)=log2(error3(j,i)/error3(j,i+1));
        cnv_rt3(i)=log2(error3(i)/error3(i+1));
 end
%end

cnv_rt1
cnv_rt2
cnv_rt3
error1
error2
error3
% max(cnv_rt1)
% max(cnv_rt2)
% max(error1)
% max(error2)