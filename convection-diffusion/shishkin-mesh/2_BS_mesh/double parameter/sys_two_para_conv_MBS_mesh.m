...
for N_counter =1:4
   ...
   % Nt1=4*2^(N_counter-1);
    %Nx1=Nt1^2;
     Nt1=16*2^(N_counter-1);
    Nx1=2*Nt1;
    
%sig2=min(1/2,4.2*e2*log(Nx1));
%sig1=min(sig2/2,4.2*(e1)*log(Nx1));
% sig2=0.0024;
% sig1=0.0014;
for l=1:2
     Nx=Nx1*2^(l-1);
...
for i=1:(Nx/4)
    x(i+1)=-2*e1*log(exp(-(i/Nx)/(((1/4)+(1/(4*log(Nx))))-(i/Nx))));
end
for i=(Nx/4)+1:(Nx/2)
    x(i+1)=x((Nx/4)+1)-2*(e2-e1)*log(exp(-(((i/Nx)-1/4)/(((1/4)+(1/(4*log(Nx))))-((i/Nx)-(1/4))))));
