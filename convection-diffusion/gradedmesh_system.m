...
for i=(Nx/2)+2:Nx-1
    x1(i+1)=x1(i)+1/3*Nx^(-1)*log((Nx)*x1(i));
 end
x1(Nx+1)=1;
...
for i=(Nx/2)+2:Nx-1
    x2(i+1)=x2(i)+1/3*Nx^(-1)*log((Nx)*x2(i));
 end
x2(Nx+1)=1;
...
%      U1(i,1)=0;
%      U2(i,1)=0;
    U1(i,1)=(c1*(1-exp(-x1(i)/e1))-x1(i));
     U2(i,1)=(c2*(1-exp(-x2(i)/e2))-x2(i));
...
%%%%%%%%%%%%%%construction of finite diffrence matrix A B C D M
 ...
 for j=2:Nt+1
 for i=2:Nx-2
     A(i,i-1)=-2*e1*k/((h1(i+1)+h1(i))*h1(i));
     A(i,i)=(2*e1*k/(h1(i+1)*h1(i)))+k/h1(i+1)+k*(1+x1(i))+1;
     A(i,i+1)=(-2*e1*k/((h1(i+1)+h1(i))*h1(i+1)))-k/h1(i+1);
...
%      A(i,i+1)=(-2*e1*k/((h(i+1)+h(i))*h(i+1)))-k/h(i+1);
 ...
 for i=2:Nx-2
     D(i,i-1)=-2*e2*k/((h2(i+1)+h2(i))*h2(i));
     D(i,i)=(2*e2*k/(h2(i+1)*h2(i)))+k/h2(i+1)+k*(1+2*(x2(i)))+1;
      D(i,i+1)=(-2*e2*k/((h2(i+1)+h2(i))*h2(i+1)))-k/h2(i+1);
