...
for l1=1:2
    ...
    Ny1=16*2^(l1-1);
    Nt1=4*2^(l1-1);
  sigx=min(1/2,2*e*log(Nx1));
  sigy=min(1/2,2*e*log(Ny1));
...
for i=(Nx/2)+1:Nx+1
    x(i)=1+2*e*log(((Nx^2)-2*(Nx-i)*(Nx-1))/(Nx^2));
end
%x(Nx+1)=1;
...
for j=(Ny/2)+1:Ny+1
    y(j)=1+2*e*log(((Ny^2)-2*(Nx-j)*(Ny-1))/(Ny^2));
end
 %y(Ny+1)=1;
...
it;
    ...
    for j=1:Ny-1
      ...
      for i=2:Nx-2
        ...
        A1(i,i)=1+(2*e*ht)/(hx(i)*hx(i+1))+ht*(1+x(i)*y(j)/2)/hx(i)+ht*(4+2*t(k)*(x(i)^2)+2*t(k)*(y(j)^2))/2;  % a11=a11/2+a11/2 as defined for fractional step
