...
e2=2^(-6);
 ...
 c1=1/(1-exp(-1/e1));
  c2=1/(1-exp(-1/e2));
sig2=min(1/2,1*e2*log(M));
sig1=min(sig2/2,1*(e1)*log(M));
...
%%%%%%%%%%%%% Exact Solution %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
 for i=1:M
     ...
     % U2_exct(i)=c1*(1-exp(-(1-x(i))/e1))+c2*(1-exp(-(1-x(i))/e2))-2*sin(pi*(1-x(i))/2);
     %U2_exct(i)=c2*(1-exp(-(1-x(i))/e2))-(1-x(i))*exp(-x(i));
     U2_exct(i)=c2*(1-exp(-(1-x(i))/e2))-(1-x(i));
 end
    U1_exct(M+1)=0;
    U2_exct(M+1)=0;  
